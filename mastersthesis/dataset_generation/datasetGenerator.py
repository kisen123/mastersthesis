...
noiseStd : float  ->  less than 0, determines how noisy the image gets
...
def circMask(h, w):
    ...
    # Create a black image with a boolean circle in the middle
    img = np.zeros((h, w), dtype=np.uint8)
    radius = int(np.floor(h / 2))
    cv2.circle(img, (int(np.floor(h / 2)), int(np.floor(w / 2))), radius, 255, -1)

    # Convert the image to a boolean mask
    circMaskArray = img.astype(bool)

    return circMaskArray
#%%
...
def makeImage(codeDir,
    ...
    fig, ax = plt.subplots(figsize=(dimX/my_dpi, dimY/my_dpi), dpi=my_dpi)
    ...
    for ID, samples in enumerate(numSamples):
        ...
        for sample in range(samples):
            ...
            for i, (x, y) in enumerate(speckleCoordinates):
                ...
                rectangle = Rectangle((x, y), resizeFactor * randomWidthsHeights[i, 0], 
                                      resizeFactor * randomWidthsHeights[i, 1], randomAngles[i], color='black',
                                      alpha=blackness[i])
            ...
            ax.scatter(dimX/2, dimY/2, s=resizeFactor * 5, c='black', alpha=0.95)
            
            # Making a black line to set orientation
            middlePoint = (round(dimX/2), round(dimY/2))
            
            digestivePosition = np.array([dimX/2 - 1, dimY/1.5])
            ...
            digestiveRectangle = Rectangle(digestivePosition, dimY * 0.02,
                                           dimY * 0.5, angleRotation,
                                           color='black', alpha=digestiveBlack)
            ...
            # Set the figure size to match the size of the image
